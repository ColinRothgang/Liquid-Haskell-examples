...
Proof.
 revert n. induction m as [| m IHm ]. intros n; now smt_trivial. intros n; smt_app (IHm n).
Qed.
...
Proof.
  revert n o. induction m as [| m IHm ]. intros n o; now smt_trivial. intros n o; smt_app (IHm n o).
Qed.

Theorem mult_suc_r (n: N) (m: N): (` (mult n (Suc m))) = (` (add n (` (mult n m)))).
Proof.
 revert m. induction n as [| n IHn ]. intros m; now smt_trivial. intros m; smt_app (IHn m); smt_app (add_assoc m n (` (mult n m))); smt_app (add_comm m n); smt_app (add_assoc n m (` (mult n m))).
Qed.
